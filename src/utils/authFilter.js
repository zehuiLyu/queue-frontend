...
/**
 * 【 subtable row editing 】 Achieve two functions:
 * 1, hide JEditableTable without permission columns
 * 2, disable JEditableTable without permission columns
 * @param columns
 * @param pre
 * @returns {*}
 */
export function colAuthFilterJEditableTable(columns,pre) {
  ...
  const cols = columns.filter(item => {
    ...
    //Code strict processing, prevent one authorization mark, configure multiple times
...
// If the permission is invalid, do not control it. If the permission is valid, do not control it
// Obtain unauthorized column information based on the authorization code prefix
...
/**
 * Add the line edit button permission Online to local storage
 */
...
/**
 * Add additional methods [for line editing components]
 * @param pre
 * @returns {*[]}
 */
